...
LegacyReporter::LegacyReporter(const UrlCopyOpts &opts): producer(opts.msgDir), opts(opts),
    zmqContext(1), zmqPingSocket(zmqContext, ZMQ_PUB) // counterSocket(cxt, XXX)
{
    std::string address = std::string("ipc://") + opts.msgDir + "/url_copy-ping.ipc";
    zmqPingSocket.connect(address.c_str());

    // bind connect the socket tcp://hostname:port
    // first step, hard code hostname:port, then from config and then from directory
}
...
{
    // super::sendTra
    // counterSocket.send(), simple super will need to constriuct event twice
    // send need to define protocolbuf
