...
std::list<Pair> MySqlOptimizerDataSource::getActivePairs(void) {
    FTS3_COMMON_LOGGER_NEWLOG(INFO) << "DEV: MySqlOptimizerDataSource getActivePairs" << commit;
...
void MySqlOptimizerDataSource::setNewOptimizerValue(soci::session &sql,
...
// the progress
void MySqlOptimizerDataSource::updateOptimizerEvolution(soci::session &sql,
...
// Only non terminal!
int MySqlOptimizerDataSource::getCountInState(soci::session &sql, const Pair &pair, const std::string &state)
{
    int count = 0;
